...
이 길은 해의 수만큼 풀어야 하고, 막는 절이 쌓일수록 남은 해를 찾기가 어려워진다.
처음에 이 꾸러미의 |Solve|는 부를 때마다 절 전체로 자료 구조를 새로 짓고 배운 것을
모두 잊었다. 그때는 10-퀸의 724개를 세는 데 mem 200억 개, 23초가 걸렸다. 지금의
|Solve|는 절을 더하고 다시 부르면 배운 절과 활동도, 지난 해의 값을 간직한 채 이어 푼다.
지난 해 바로 곁에서 찾기 시작하니 다음 해가 금방 나온다. 내 컴퓨터에서 8-퀸의 92개는
mem 585만 개, 0.01초에 세고, 10-퀸의 724개는 mem 2억 5천만 개, 0.25초에 센다. 그래도
해를 하나씩 세는 길이니, 해가 수백만 개로 늘면 해를 한꺼번에 세는 BDD를 따라갈 수 없다.
